...
where A. idThietBi= C.idThietBi and C.idPhieuMuon= D.idPhieuMuon and A.idLoaiTB=B.idLoaiTB and HanTra<= GETDATE() +5 

/* -	Liệt kê các thiết bị đã quá hạn trả*/
select tenTB, HanTra, TenLoaiTB from ThietBi A, LoaiTB B, CT_PhieuMuonTra C, PhieuMuonTra D
where A. idThietBi= C.idThietBi and C.idPhieuMuon= D.idPhieuMuon and A.idLoaiTB=B.idLoaiTB and HanTra< GETDATE()
...
def get_muon_toihan(session: Session, n: int) -> List[LietkeMuonInfo]:
    result = session.execute("""select tenTB, HanTra, TenLoaiTB from ThietBi A, LoaiTB B, CT_PhieuMuonTra C, PhieuMuonTra D
        where A. idThietBi= C.idThietBi and C.idPhieuMuon= D.idPhieuMuon and A.idLoaiTB=B.idLoaiTB and HanTra<= CURRENT_DATE() + {}""".format(n)).all()
    if result is None:
        raise TBInfoNotFoundError
    print(result)
    return  result
...
def get_quahan(session: Session) -> List[LietkeMuonInfo]:
    result = session.execute("""select tenTB, HanTra, TenLoaiTB from ThietBi A, LoaiTB B, CT_PhieuMuonTra C, PhieuMuonTra D
where A. idThietBi= C.idThietBi and C.idPhieuMuon= D.idPhieuMuon and A.idLoaiTB=B.idLoaiTB and HanTra< CURRENT_DATE() """).all()
    if result is None:
        raise TBInfoNotFoundError
    print(result)
    return  result
